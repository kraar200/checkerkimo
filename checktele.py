...
@kimo.on(events.NewMessage(outgoing=True, pattern=r"\.تشيكر"))
async def _(event):
    if ispay2[0] == "yes":
        await event.edit(tele_checker)
        
@kimo.on(events.NewMessage(outgoing=True, pattern=r"\.اليوزرات المبندة"))
async def _(event):
    if ispay2[0] == "yes":
        await kimo.send_file(event.chat_id, 'banned.txt')


@kimo.on(events.NewMessage(outgoing=True, pattern=r"\.الانواع"))
async def _(event):
    if ispay2[0] == "yes":
...
@kimo.on(events.NewMessage(outgoing=True, pattern=r"\.صيد (.*)"))
async def _(event):
    if ispay2[0] == "yes":
        isclaim.clear()
        isclaim.append("on")
        ...
        @kimo.on(events.NewMessage(outgoing=True, pattern=r"\.حالة الصيد"))
        async def _(event):
            if ispay2[0] == "yes":
                if "on" in isclaim:
        ...
        for i in range(int(msg[0])):
            ...
            if "Available" in isav:
                ...
                try:
                    await kimo(functions.channels.UpdateUsernameRequest(
...
CH / @d8_8q 
    ''')
                    ...
                    await kimo.send_message(event.chat_id, f'''خطأ مع {username}
    الخطأ :
    {str(eee)}''')
                    if "A wait of" in str(eee):
                        break
                    else:
                        await kimo.send_message(event.chat.id, " اجاك متاح !")
            else:
                pass
            trys += 1
...
@kimo.on(events.NewMessage(outgoing=True, pattern=r"\.تثبيت (.*)"))
async def _(event):
    if ispay2[0] == "yes":
        trys = 0
        ...
        if msg[0] == "تلقائي":  # تثبيت تلقائي عدد يوزر قناة
            ...
            @kimo.on(events.NewMessage(outgoing=True, pattern=r"\.حالة التثبيت التلقائي"))
            async def _(event):
                if "on" in isauto:
...
CH / @d8_8q 
    ...
    {str(eee)}''')
            ...
            isclaim.clear()
            isclaim.append("off")
            await kimo.send_message(event.chat_id, "تم الانتهاء من التثبيت التلقائي")
